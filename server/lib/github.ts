import { Octokit } from "@octokit/rest";
  ...
  if (!token) {
    console.error("ERROR: GITHUB_PERSONAL_ACCESS_TOKEN is not set. The server will not be able to connect to GitHub.");
    console.error("Please set this environment variable with a GitHub PAT that has 'repo' scope.");
  }

  if (!owner) {
    console.error("ERROR: GITHUB_OWNER is not set. Please set this to your GitHub username or organization.");
  }

  if (!repo) {
    console.error("ERROR: GITHUB_REPO is not set. Please set this to the target repository name.");
  }
